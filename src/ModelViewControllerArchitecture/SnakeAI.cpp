/**
 * @file SnakeAI.cpp
 * @brief Implementation of Deep Q-Network Snake AI with Real-time Visualization
 * @author moinshaikh
 * @date 1/19/26
 * 
 * This file implements the complete DQN training system including:
 * - Neural network forward propagation and training
 * - Snake game environment simulation
 * - Experience replay and target network updates
 * - Real-time SDL3 visualization with interactive parameter tuning
 * 
 * Key Algorithms Implemented:
 * 1. Deep Q-Learning with experience replay
 * 2. Epsilon-greedy exploration strategy
 * 3. Target network for stable training
 * 4. Shaped reward function for efficient learning
 * 
 * Mathematical Foundations:
 * - Q-learning update: Q(s,a) ← Q(s,a) + α[r + γ·max_a'Q(s',a') - Q(s,a)]
 * - Experience replay sampling from uniform distribution
...
/**
 * @brief Q-Network Constructor
 * 
 * Initializes the three-layer neural network with proper weight registration.
 * Each layer is registered with PyTorch's autograd system for gradient computation.
 * 
 ...
 * 
 * Weight matrices are initialized using PyTorch's default initialization
...
/**
 * @brief Forward pass through the Q-network
 * 
 * @param input Input tensor representing current state [batch_size, 16]
 * @return Output tensor of Q-values [batch_size, 4]
 * 
 * Computational flow:
 * 1. Linear transformation: h1 = x·W1 + b1, where W1 ∈ ℝ^(16×128)
 * 2. ReLU activation: a1 = max(0, h1)
 * 3. Linear transformation: h2 = a1·W2 + b2, where W2 ∈ ℝ^(128×128)
 * 4. ReLU activation: a2 = max(0, h2)
 * 5. Linear transformation: Q = a2·W3 + b3, where W3 ∈ ℝ^(128×4)
 * 
 * The output Q[i] represents the expected future reward for action i.
 * No activation on final layer allows both positive and negative Q-values.
 */
...
{
    /**
     * @brief SnakeAI Constructor - Initialize DQN training system
     * 
     * @param Render Enable/disable real-time visualization
     * 
     * Initializes all components for DQN training:
     * - Creates main and target Q-networks with identical weights
     * - Sets up Adam optimizer with initial learning rate
     * - Seeds random number generator with system time
     * - Synchronizes target network with main network
     * 
     * The constructor ensures both networks start with identical weights,
     * which is crucial for stable DQN training. The target network
    ...
    {
        // Seed Mersenne Twister with high-precision system time
     ...
     * 
     * Properly deallocates SDL3 resources in reverse order of creation.
     * Prevents memory leaks and ensures clean shutdown.
     * 
     * Cleanup order:
     * 1. Destroy renderer (frees drawing context)
     * 2. Destroy window (frees window resources)
     * 3. Quit SDL subsystems (frees global state)
     */
    SnakeAI::~SnakeAI()
    {
     ...
     * 
     * @return State vector encoding complete game information
     * 
     ...
     * 
     * This compact representation captures all essential information
     * for effective decision making while keeping network size manageable.
     */
     ...
     * During exploitation, neural network selects optimal action via argmax.
     * 
...
/**
     ...
     * @param shouldPrint Enable debug output (unused)
     * @return Immediate reward for this step
     * 
     * Implements complete game physics and reward calculation:
     * 1. Convert action to direction, prevent 180° turns
     * 2. Calculate new head position based on direction
     * 3. Check collision conditions (wall, self, timeout)
     * 4. Calculate Manhattan distance to food before/after move
     * 5. Apply reward shaping based on movement efficiency
     * 6. Handle food consumption and snake growth
     * 
     * Reward function R(s,a,s'):
     * - +10.0: Eating food (primary objective)
     * - +0.1: Moving closer to food (shaping reward)
     * - -0.15: Moving away from food (shaping penalty)
     * - -10.0: Death (wall/self-collision/timeout)
     * - -5.0: Timeout (half death penalty)
     * 
     * Timeout formula: timeout = 50 + 10 × snake_length
    ...
    float SnakeAI::step(int action, bool shouldPrint)
    {
     if (game_over) {
        ...
        // Prevent 180° turns (opposite directions sum to 0)
        ...
        // Calculate Manhattan distance to food before and after move
        ...
        // Check timeout (prevents infinite loops)
        if(stepWithotfood > 50 + 10 * (int)snake.size()){
            game_over = true;
        ...
        if(!ate)
            {
            snake.pop_back();
            // Reward shaping: encourage efficient movement toward food
     ...
     * Core of the DQN learning algorithm.
     * 
     * Training algorithm:
     * 1. Sample random minibatch of experiences from replay buffer
     * 2. For each experience (s,a,r,s',done):
     *    - Compute current Q-values: Q(s,a;θ)
     *    - Compute target Q-values:
     *      y = r, if done
     *      y = r + γ·max_a'Q(s',a';θ^-), otherwise
     * 3. Compute loss: L = MSE(Q(s,a;θ), y)
     * 4. Backpropagate gradients and update parameters with Adam
     * 5. Every 50 steps: update target network: θ^- ← θ
     * 
     * Mathematical formulation:
     * Loss: L(θ) = E[(y - Q(s,a;θ))^2]
     * Target: y = r + γ·max_a'Q(s',a';θ^-)·(1-done)
     * 
     * Target network provides stable targets, preventing divergence
     * caused by moving target problem in standard Q-learning.
     */
    void SnakeAI::train_step()
    {
        ...
        // Update target network every 50 training steps
        static int c = 0; c++;
        if(c % 50 == 0)
        {
...
/**
     ...
     * 
     * Implements complete DQN training algorithm with real-time visualization:
     * 
     * Training loop structure:
     * for episode = 1 to episodes:
     *   1. Reset environment, get initial state s₀
     *   2. while not terminal and steps < 1000:
     *      a. Select action via ε-greedy: aₜ = π(sₜ)
     *      b. Execute action: (sₜ₊₁, rₜ, done) = env.step(aₜ)
     *      c. Store experience: replay_buffer.push((sₜ,aₜ,rₜ,sₜ₊₁,done))
     *      d. Train network if buffer ≥ batch_size
     *      e. Update state: sₜ ← sₜ₊₁
     *      f. Handle user input for parameter tuning
     *      g. Render if conditions met
     *   3. Update statistics and decay exploration
     * 
     * Key features:
     * - Real-time parameter adjustment via keyboard
     * - Adaptive rendering based on train_speed
     * - Comprehensive statistics tracking
     * - Epsilon decay for exploration-exploitation balance
     * 
     * Performance metrics tracked:
     * - Episode scores (individual and averaged)
     * - Maximum score achieved
     * - Exploration rate (epsilon) history
    ...
    void SnakeAI::train(int episodes){
        ...
        for(int ep = 0; ep < episodes; ep++){
            ...
            while(!game_over && steps < 1000){
                ...
                // Train neural network if enough experiences available
                ...
                // Render based on train_speed setting
